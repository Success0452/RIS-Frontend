...
// @ts-ignore
export const DashboardCustomHeader = ({children, setIsLoading}) => {
    const router = useRouter();
    const [open, setOpen] = useState(false);
    return (
        <div className="flex flex-row items-start justify-start md:pl-6 md:pr-6 pr-3 pl-3 w-full min-h-screen">
            {!open && <DashDesktopItem open={open} setOpen={setOpen} setIsLoading={setIsLoading} >{children}</DashDesktopItem>}
